...
Auto-generated by nodeconfig on first boot. Each Pi gets a unique `PI_NUMBER` and `MESH_IP`. See `.env.example` at the root of the repo for the full template — the values surfaced below are the ones an operator typically tunes.
...
AP_CHANNEL=0                   # 0 = ACS auto-pick from {1, 6}, staggered by AP MAC
...
# ── IRC Federation ──
IRC_LINK_PASSWORD=<set-during-install>          # IDENTICAL on all nodes

# ── Tailscale (optional remote SSH) ──
TAILSCALE_AUTH_KEY=
```

> **Note:** Configs (`hostapd.conf`, `dnsmasq.conf`) are regenerated from `.env` on every `nightwatch-mesh.service` start, so editing `.env` and running `systemctl restart nightwatch-mesh` is enough to apply changes. The chat banner reads its values via `/api/config` and updates without a restart on page refresh.

### Legal disclosures for public installations
